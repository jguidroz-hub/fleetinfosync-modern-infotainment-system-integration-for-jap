...
interface DeviceConnectionsItem {
  id: string;
  title?: string;
  status?: string;
  createdAt: string;
...
export default function DeviceConnectionsPage() {
  const [items, setItems] = useState<DeviceConnectionsItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [showCreate, setShowCreate] = useState(false);
  const [newTitle, setNewTitle] = useState('');

  useEffect(() => {
    fetch('/api/device-connections')
      .then(r => r.json())
  ...
  const handleCreate = async () => {
    ...
    const res = await fetch('/api/device-connections', {
      method: 'POST',
  ...
  const handleDelete = async (id: string) => {
    await fetch(`/api/device-connections/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Device Integrations</h1>
        <button
      ...
      <p className="text-gray-600 mb-6">Smartphone and vehicle connection management</p>
